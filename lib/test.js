(function() {
  ...
  this["use partial freezing (1/3)"] = function() {
    ...
    d2 = lets(d, function(d_copy) {
      assert.ok(d !== d_copy, '^lft@48^');
      assert.ok(!is_readonly(d_copy, 'nested'), '^lft@49^');
      assert.ok(!is_readonly(d_copy, 'u'), '^lft@50^');
      assert.ok(!is_readonly(d_copy.u, 'v'), '^lft@51^');
      assert.ok(!is_readonly(d_copy.u.v, 'w'), '^lft@52^');
      assert.ok(!Object.isSealed(d_copy), '^lft@53^');
      assert.ok(!Object.isSealed(d_copy.nested), '^lft@54^');
      assert.ok(!Object.isSealed(d_copy.u), '^lft@55^');
      assert.ok(!Object.isSealed(d_copy.u.v), '^lft@56^');
      try {
      ...
      try {
        ...
        e = error1;
        throw new Error('^lft@59^ ' + e.message);
      }
      try {
    ...
    assert.ok(d2 !== d, '^lft@61^');
    assert.deepEqual(d, matcher_a, '^lft@62^');
    assert.deepEqual(d2, matcher_c, '^lft@63^');
    //.........................................................................................................
    d_thawed = thaw(d);
    assert.deepEqual(d_thawed, d, '^lft@64^');
    assert.ok(d !== d_thawed, '^lft@65^');
    assert.ok(!is_readonly(d_thawed, 'nested'), '^lft@66^');
    assert.ok(!is_readonly(d_thawed, 'u'), '^lft@67^');
    assert.ok(!is_readonly(d_thawed.u, 'v'), '^lft@68^');
    assert.ok(!is_readonly(d_thawed.u.v, 'w'), '^lft@69^');
    assert.ok(!Object.isSealed(d_thawed), '^lft@70^');
    assert.ok(!Object.isSealed(d_thawed.nested), '^lft@71^');
    assert.ok(!Object.isSealed(d_thawed.u), '^lft@72^');
    assert.ok(!Object.isSealed(d_thawed.u.v), '^lft@73^');
    try {
    ...
    try {
      ...
      e = error1;
      throw new Error('^lft@76^ ' + e.message);
    }
    try {
